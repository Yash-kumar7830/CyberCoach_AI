...
/* ── Static question bank per training+role (sample 3 questions each) ──
   In production replace with API call or larger JSON. */
const QUESTIONS = {
  "finance-employee": [
    {
      scenario: "You receive an urgent email from your CEO asking you to wire ₹5,00,000 to a new vendor. The email looks legit but arrived on a Sunday.",
      question: "What is the MOST appropriate first step?",
      options: ["Wire the funds immediately to avoid delays", "Call the CEO on their official number to verify", "Forward the email to Finance for processing", "Reply asking for more details"],
      correct: 1,
      explanation: "CEO/BEC fraud relies on urgency and authority. Always verify via a separate, trusted communication channel before any transfer.",
      level: "Beginner",
    },
    {
      scenario: "An invoice arrives from a regular supplier, but the bank account number has changed. The supplier says they updated their banking details.",
      question: "How should you validate this change?",
      options: ["Update the bank details and process the invoice", "Email the supplier to confirm", "Call the supplier on a number from your existing records — NOT the invoice", "Ask your manager to approve the change"],
      correct: 2,
      explanation: "Attackers intercept supplier emails and change bank details. Always call using a verified number from your own records, never from the suspicious document.",
      level: "Intermediate",
    },
    {
      scenario: "You notice a colleague accessing the finance system at 2 AM from an overseas IP, which is unusual for them.",
      question: "What action should you take?",
      options: ["Ignore it — they may be travelling", "Send them a friendly message asking if they're okay", "Report it immediately to IT Security as a potential account compromise", "Check again tomorrow to see if it continues"],
      correct: 2,
      explanation: "Unusual access patterns are a key indicator of account compromise. Immediate reporting allows timely containment.",
      level: "Advanced",
    },
  ],
  "phishing-employee": [
    {
      scenario: "You get an email: 'Your Microsoft 365 account will be suspended in 24 hours. Click here to verify.' The link shows 'micros0ft-support.net'.",
      question: "What is the red flag here?",
      options: ["The 24-hour urgency is normal for IT", "The domain name uses '0' instead of 'o' — it's spoofed", "Microsoft always sends suspension notices", "The email format looks legitimate"],
      correct: 1,
      explanation: "Typosquatting — replacing letters with lookalikes — is a classic phishing technique. Always inspect the full domain before clicking.",
      level: "Beginner",
    },
    {
      scenario: "A LinkedIn message from a 'recruiter' asks you to fill a form to 'pre-screen' for a dream job. The form requests your current employer email and password.",
      question: "This is an example of:",
      options: ["Legitimate recruiting process", "Spear phishing / credential harvesting", "A standard background check", "HR data collection"],
      correct: 1,
      explanation: "Legitimate recruiters never ask for your current employer credentials. This is a targeted spear-phishing attack using social proof (job opportunity).",
      level: "Beginner",
    },
    {
      scenario: "You clicked a suspicious link by mistake on your work laptop. The page loaded for a second then went blank.",
      question: "What should you do immediately?",
      options: ["Clear your browser cache and carry on", "Disconnect from the network and report to IT", "Run a quick antivirus scan and continue working", "Restart your laptop"],
      correct: 1,
      explanation: "Drive-by downloads can execute in milliseconds. Isolating the device prevents lateral movement across the network.",
      level: "Intermediate",
    },
  ],
  "it-security-it-admin": [
    {
      scenario: "A developer requests local admin rights on their workstation, claiming it's needed to install a testing library.",
      question: "Best practice response?",
      options: ["Grant permanent admin rights for convenience", "Deny — no one needs admin rights", "Grant time-limited admin using a PAM tool, then auto-revoke", "Ask the developer to submit a ticket and approve later"],
      correct: 2,
      explanation: "Principle of least privilege with just-in-time access minimises attack surface while enabling legitimate work.",
      level: "Advanced",
    },
    {
      scenario: "A critical CVE (CVSS 9.8) is published for software running on 40 production servers. It has no patch yet.",
      question: "Immediate mitigation priority?",
      options: ["Wait for the official patch", "Apply compensating controls: WAF rules, network segmentation, disable unused features", "Notify users and monitor logs", "Take servers offline until patch is available"],
      correct: 1,
      explanation: "When no patch exists, layered compensating controls (virtual patching via WAF, network isolation) reduce exposure while maintaining availability.",
      level: "Advanced",
    },
    {
      scenario: "Monitoring alerts show 1,000 failed SSH login attempts from a single IP in 5 minutes, followed by one successful login.",
      question: "This pattern indicates:",
      options: ["A misconfigured automated tool", "A successful brute-force attack — treat as active compromise", "Normal user behaviour", "A network scanner running a security audit"],
      correct: 1,
      explanation: "Brute-force success is an active incident. Immediately isolate the account, revoke sessions, and begin IR procedures.",
      level: "Intermediate",
    },
  ],
};

/* Fallback questions for any unmapped combination */
const FALLBACK_QUESTIONS = [
  {
    scenario: "You receive a phone call from 'IT Support' asking for your login credentials to fix an urgent system issue.",
    question: "What should you do?",
    options: ["Provide credentials to resolve the issue quickly", "Hang up and call IT Support using the official number", "Give partial credentials to verify their identity", "Ask for their employee ID and comply"],
    correct: 1,
    explanation: "Vishing (voice phishing) is common. Legitimate IT support will never ask for your credentials over the phone.",
    level: "Beginner",
  },
  {
    scenario: "You find a USB drive in the company car park labelled 'Salary_2024.xlsx'.",
    question: "What is the safest action?",
    options: ["Plug it in to check if it belongs to a colleague", "Take it home to check on a personal PC", "Hand it to IT Security without plugging it in", "Leave it where you found it"],
    correct: 2,
    explanation: "Baiting attacks rely on curiosity. Plugging in unknown drives can execute malware instantly. Always hand to IT.",
    level: "Beginner",
  },
  {
    scenario: "Your company's password policy requires 8-character passwords changed every 30 days. Users are writing them on sticky notes.",
    question: "What is the underlying security problem?",
    options: ["Users need more security training", "The policy creates friction, leading to insecure workarounds — policy needs redesign", "Sticky notes should be banned", "Passwords are too complex"],
    correct: 1,
    explanation: "Overly strict, frequent rotation policies backfire. NIST recommends long passphrases changed only on suspected compromise, combined with MFA.",
    level: "Intermediate",
  },
];
...
function StepIndicator({ step }) {
  const steps = ["Select Role", "Quiz", "Results"];
  return (
...
export default function Quiz() {
  ...
  const [step,        setStep]        = useState(0); // 0=role select, 1=quiz, 2=results
    ...
    <main className="min-h-screen bg-gray-950 text-white" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-12">
        ...
        <div className="flex items-center gap-2 text-xs text-gray-600 mb-8">
          <Link to="/quiz/general" className="hover:text-green-400 transition-colors">Training Hub</Link>
          <span>/</span>
        ...
        <div className="mb-8 text-center">
          ...
          <h1 className="syne text-2xl sm:text-3xl font-extrabold text-white">
            {step === 0 ? "Select your role" : step === 1 ? "Scenario Quiz" : "Your Results"}
          </h1>
        </div>
        ...
        {step === 0 && (
          <div className="fade-up">
            ...
            <div className="rounded-xl border border-gray-800/60 bg-gray-900/30 p-5 mb-6">
              <h3 className="syne text-sm font-bold text-white mb-3">Quiz levels in this module</h3>
